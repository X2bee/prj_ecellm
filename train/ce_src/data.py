...
class TrainDatasetForCE(Dataset):
    ...
    def __getitem__(self, item) -> List[BatchEncoding]:
        ...
        if len(self.dataset[item]['negative']) < self.args.train_group_size - 1:
            ...
            negs = random.sample(self.dataset[item]['negative'] * num, self.args.train_group_size - 1)
        else:
            negs = random.sample(self.dataset[item]['negative'], self.args. train_group_size- 1)
